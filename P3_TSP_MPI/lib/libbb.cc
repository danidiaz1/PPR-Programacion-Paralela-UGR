...
// Comunicadores que usará cada proceso
...
void Equilibrar_Carga(tPila & pila, bool & fin, tNodo & sol){

  MPI_Status status;
  int count, source, tag, flag, solicitante;
  tNodo sol_temporal;
  color = BLANCO;

  if (pila.vacia()){ // El proceso no tiene trabajo: pide a otros procesos
    // Enviar petición de trabajo al siguiente proceso
    ...
    while (pila.vacia() && !fin){
      ...
      switch (tag) {
        case NODOS: // Resultado de una petición de trabajo
          // Recibir nodos del proceso donante y almacenarlos en la pila
        ...
        case PETICION: // Peticion de trabajo
          ...
          if (solicitante == rank){ // Petición devuelta
            estado = PASIVO; // puede que no quede trabajo por hacer

            if (token_presente) {
              if (rank == 0) color_token = BLANCO;
              else color_token = color;

              // Empezamos a circular el token de detección de fin
          ...
          case TOKEN: // Mensaje de token de mecanismo de detección de fin
            MPI_Recv(NULL, 0, MPI_INT, siguiente, TOKEN, comunicadorCarga, &status);
            token_presente = true;

            // si el proceso está sin trabajo
            if (estado == PASIVO){
              ...
              if (rank == 0 && color == BLANCO && color_token == BLANCO){
                ...
                // Esperamos hasta que todos hayan recibido el mensaje de fin
                MPI_Recv(sol_temporal.datos, 2*NCIUDADES, MPI_INT, anterior, FIN, comunicadorCarga, &status);
                // Comparamos las soluciones, al final el 0 se queda con la mejor solucion
                if (sol_temporal.esMejorSol(sol)) CopiaNodo(&sol_temporal, &sol);
              }
              else {
                if (rank == 0) color_token = BLANCO;
                else color_token = color;
          ...
          case FIN: // Mensaje de fin, todos los procesos se quedan sin nodos
            MPI_Recv(sol_temporal.datos, 2*NCIUDADES, MPI_INT, anterior, FIN, comunicadorCarga, &status);
            fin = true;
            // Compara la solucion del proceso con la que le ha llegado
            if (sol_temporal.esMejorSol(sol)) CopiaNodo(&sol_temporal, &sol);
            MPI_Send(sol.datos, 2*NCIUDADES, MPI_INT, siguiente, FIN, comunicadorCarga);
            break;
      }
...
void Difusion_Cota_Superior(int & cota_superior, bool & hay_nueva_cs)
{
  difundir_cs_local = hay_nueva_cs;
  ...
  if (difundir_cs_local && !pendiente_retorno_cs) {
    ...
    MPI_Send(&cota_superior, 1, MPI_INT, siguiente, rank, comunicadorCota);
  ...
  while ( flag > 0 ){ // mientras haya mensajes
    ...
    if (nueva_cs < cota_superior){
      cota_superior = nueva_cs;
      hay_nueva_cs = true;
    }

    if (tag == rank){ // origen del mensaje == mismo proceso
      ...
      MPI_Send(&cota_superior, 1, MPI_INT, siguiente, tag, comunicadorCota);
    }
